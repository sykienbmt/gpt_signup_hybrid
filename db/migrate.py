    ...
    def migrate_outlook_state(self, state_dir: Path) -> MigrationSummary:
        ...
        with self._engine.get_connection() as conn:
            for filepath in json_files:
                ...
                if existing:
                    logger.warning("skip duplicate: %s", email)
    ...
    def migrate_sessions(self, sessions_dir: Path) -> MigrationSummary:
        ...
        with self._engine.get_connection() as conn:
            for filepath in json_files:
                ...
                # Check duplicate — email + created_at
                existing = conn.execute(
                    "SELECT id FROM session_results WHERE email = ? AND created_at = ?",
                    (email, created_at),
                ).fetchone()
                if existing:
                    logger.warning("skip duplicate session: %s @ %s", email, created_at)
                    summary.skipped_duplicate += 1
                    continue
