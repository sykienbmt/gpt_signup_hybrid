...
_TERMINAL_ERRORS = (
    ...
    "AADSTS90023",   # No applicable permissions (Mail.Read etc. not granted)
    "wrong password — account already exists",  # Account đã tồn tại với password khác
)
...
def mark_signup_failure(*, state_dir: Path, email: str, error: str, registered_password: str | None = None) -> None:
    ...
    # Lưu password đã dùng để register (nếu có) để retry dùng đúng password
...
def get_registered_password(*, state_dir: Path, email: str) -> str | None:
    """Lấy password đã dùng để register account (nếu account đã được tạo từ trước)."""
    state = _read_state(state_dir, email)
    return state.get("registered_password")
